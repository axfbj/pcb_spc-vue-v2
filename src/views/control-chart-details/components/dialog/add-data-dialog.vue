  ...
  >
    <div style="padding: 10px 40px;">
      <el-form ref="form" :model="form" label-width="100px" :rules="rules">
        <el-row :gutter="40">
          <el-col :span="12">
            <el-form-item label="抽检时间:" prop="inspectionDate">
              <el-date-picker
...
export default {
  ...
  data() {
    return {
      ...
      rules_data: {
        inspectionDate: { required: true, message: '请填写检验时间', trigger: 'change' },
        createDate: { required: true, message: '请填写录入时间', trigger: 'change' },
    ...
    async open({ load }) {
      ...
      if (['p', 'np'].includes(this.controlChartType)) {
        ...
        if (this.controlChartType === 'np') {
          ...
          this.rules[`value1`] = { required: true, message: ``, trigger: 'blur' }
        } else {
        ...
        this.badNames.badDefinitionTitles.forEach((item, index) => {
          ...
          this.rules[`badValue${index + 1}`] = { required: true, message: `请填写${item}数量`, trigger: 'blur' }
          const o = {}
        ...
        for (let i = 1; i <= this.sampleSize; i++) {
          ...
          this.size.push(o)
          this.rules[o.val] = { required: true, message: `请填写${o.label}`, trigger: 'blur' }
        }
      }
      if (this.flag === 'add') {
        this.$refs.form.resetFields()
