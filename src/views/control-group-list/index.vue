      ...
      <container-layout ref="control-pic-list">
        <template v-slot:btns>
          <div style="text-align: right;">
            <ki-button type="primary" :auto-loading="true" @click="cc(...arguments,'1111')">控制图设置</ki-button>
            <ki-button type="warning">单项目输入</ki-button>
            <ki-button type="danger" :auto-loading="true" @click="cc(...arguments,'1111')">删除</ki-button>
          </div>
        </template>
        <template v-slot:form>
          ...
          <el-form-item label="状态:">
            <allow-create-select v-model="value2" style="width: 100px;" />
          </el-form-item>
          <el-form-item label="编号:">
            <allow-create-select v-model="value1" style="width: 100px;" />
          </el-form-item>
        ...
        <template v-slot:footer>
          <div class="states-list">
            <span v-for="(item,index) in states_text" :key="index">
              <rectangle v-if="item.flag === 'history'" :state="item.state" />
              <round-shape v-else :state="item.state" />
              {{ item.text }}
            </span>
          </div>
        </template>
