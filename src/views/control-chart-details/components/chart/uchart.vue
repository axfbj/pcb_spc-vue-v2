...
export default {
  ...
  methods: {
    ...
    add_show(res) {
      const uChart = this.$echarts.init(this.$refs.chart)
      this.chartData = JSON.parse(JSON.stringify(this.chartData_data))
      const { cl } = res
      const { ucls, lcls } = res
      this.chartData = {
      ...
      res.inspectionRecordPoints.forEach(item => {
        ...
        const r = { 'value': item.y, 'date': item.tdate, 'remark': item.remark || '', inspectionRecordId: item.inspectionRecordId }
      ...
      uChart.setOption(this.chartOption)
      uChart.off('mousemove')
      const func = debounce((params) => {
        const pointInPixel = [params.offsetX, params.offsetY]
        if (uChart.containPixel('grid', pointInPixel)) {
          const xIndex = uChart.convertFromPixel({ seriesIndex: 0 }, [params.offsetX, params.offsetY])[0]
          const current_ucl = ucls[xIndex]
          const current_lcl = lcls[xIndex]
          const option = uChart.getOption()
          option.series[1].markLine.data[0].yAxis = current_ucl
          option.series[1].markLine.data[0].label.formatter = '控制上限 ' + current_ucl
          option.series[2].markLine.data[0].yAxis = current_lcl
          option.series[2].markLine.data[0].label.formatter = '控制下限 ' + current_lcl
          uChart.setOption({
      ...
      uChart.getZr().on('mousemove', (params) => {
        func(params)
      })
      this.charts = []
      this.charts.push(uChart)
